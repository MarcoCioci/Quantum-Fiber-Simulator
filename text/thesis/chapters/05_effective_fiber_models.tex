...
In the global model, the entire bipartite state is transformed according to
...
where the depolarization acts collectively on the full two-qubit system.

\medskip

In contrast, the local model introduced here acts independently on the two physical propagation arms through the tensor-product channel
...
The phase parameter \( \theta \) modifies the relative orientation of the state with respect to the chosen Bell reference basis and may therefore reduce fidelity even in the absence of entanglement degradation. In contrast, the depolarization factor \( \eta \) reduces the coherence strength of the bipartite correlations themselves.
