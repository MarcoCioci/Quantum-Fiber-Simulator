...
%   numerical tolerance. If all checks pass, a success message is printed.
    ...
    assert(abs(fidelity_self - 1.0) < tol, ...
    ...
    assert(abs(fidelity_orthogonal - 0.0) < tol, ...
    ...
    assert(abs(fidelity_phase - fidelity_expected) < tol, ...
    ...
    assert(abs(fidelity_density_pure - 1.0) < tol, ...
    ...
    assert(abs(fidelity_pure_density - 1.0) < tol, ...
        'test_fidelity:PureVsDensityFailed', ...
        'Fidelity of Bell pure state with Bell density matrix should be 1.');

    assert(abs(fidelity_density_pure - fidelity_pure_density) < tol, ...
    ...
    assert(abs(fidelity_sep - 0.5) < tol, ...
