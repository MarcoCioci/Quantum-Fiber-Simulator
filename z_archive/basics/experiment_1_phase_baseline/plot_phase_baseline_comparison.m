...
%   - Three stacked panels with shared axis limits
    ...
    build_phase_baseline_comparison_plot(tab, data, style, palette, false);
    ...
    if do_save
        ...
        export_plot(output_path, ...
            @(parent) build_phase_baseline_comparison_plot(parent, data, style, palette, false), ...
            style);
    end
...
function tl = build_phase_baseline_comparison_plot(parent, data, style, palette, show_title)
...
%   tl - tiledlayout handle
    ...
    if show_title
        title(tl, ...
            'Experiment 1 — Correlation Validation | fixed-phase sweep', ...
