function plot_composite_ensemble_tensor_snapshot(results_case, do_save)
% PLOT_COMPOSITE_ENSEMBLE_TENSOR_SNAPSHOT
% Visualize the numerical Pauli correlation tensor for one Experiment 5 case.
%
% Objective:
%   Plot only the numerical 3x3 two-qubit Pauli correlation tensor for one
%   composite fiber channel ensemble case.
%
...
%       .case_label          - case label used in the tab title
%       .distribution_type   - distribution label used in the filename
%
...
%   None
    ...
    if nargin < 1 || nargin > 2
        error('plot_composite_ensemble_tensor_snapshot:InvalidNumInputs', ...
    ...
    if ~islogical(do_save) || ~isscalar(do_save)
        error('plot_composite_ensemble_tensor_snapshot:InvalidDoSave', ...
              'do_save must be a logical scalar.');
    end

    if ~isstruct(results_case)
        error('plot_composite_ensemble_tensor_snapshot:InvalidInputType', ...
              'results_case must be a structure.');
    end

    if ~isfield(results_case, 'correlation_tensor')
        error('plot_composite_ensemble_tensor_snapshot:MissingField', ...
              'Field "correlation_tensor" not found.');
    end

    T_num = real(results_case.correlation_tensor);

    if ~isnumeric(T_num) || ~isequal(size(T_num), [3, 3])
        error('plot_composite_ensemble_tensor_snapshot:InvalidTensorSize', ...
              'correlation_tensor must be a numeric 3x3 matrix.');
    end

    if any(~isfinite(T_num(:)))
        error('plot_composite_ensemble_tensor_snapshot:InvalidTensorValues', ...
    ...
    if isfield(results_case, 'case_label')
        case_label = char(results_case.case_label);
    else
        case_label = 'case';
    end

    if isfield(results_case, 'distribution_type')
    ...
    output_folder = [ ...
        ...
        'Quantum-Fiber-Simulator/images/experiment_5'];

    output_path = fullfile(output_folder, ...
        sprintf('experiment_5_correlation_tensor_%s.png', distribution_label_file));
    ...
    [fig, tab] = get_experiment_figure_tab( ...
        ...
        'Experiment 5 — Composite Fiber Channel Ensemble', ...
        style);
    ...
    function tl = build_plot(parent_container, show_title)
        ...
        if show_title
            title(tl, ...
                sprintf('Experiment 5 — Numerical Correlation Tensor | %s', case_label), ...
