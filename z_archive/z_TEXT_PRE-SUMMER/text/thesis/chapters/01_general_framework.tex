...
Numerical simulations are built as higher-level workflows that combine these validated components. Each experiment specifies an input state, a propagation mechanism, a set of observables or metrics, and a comparison with analytical predictions whenever available. This makes the simulator not only a numerical tool, but also a reproducibility layer connecting physical assumptions, mathematical formulation, and numerical results.
